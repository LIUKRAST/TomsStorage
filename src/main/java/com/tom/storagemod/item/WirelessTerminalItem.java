...
public class WirelessTerminalItem extends Item implements WirelessTerminal {
	...
	public WirelessTerminalItem(Item.Properties settings) {
		super(settings);
	}
	...
	public static boolean isPlayerHolding(Player player) {
		return player.getMainHandItem().is(RegisterItems.WIRELESS_TERMINAL) || player.getOffhandItem().is(RegisterItems.WIRELESS_TERMINAL);

		//Changed the direct item reference with a check of this instance, allowing developers to add their own version of this item by just extending it

		/*return player.getMainHandItem().getItem() == RegisterItems.WIRELESS_TERMINAL ||
				player.getOffhandItem().getItem() == RegisterItems.WIRELESS_TERMINAL;*/
	}
